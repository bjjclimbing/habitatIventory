...
### Produccion: build de la imagen

El flujo oficial de build esta en `scripts/build_and_export.sh` y se invoca con:
...
1. elimina `public/build`
2. ejecuta `npm install` y `npm run build` dentro de `frontend/`
3. copia `frontend/dist/*` a `public/build/`
4. construye la imagen Docker con tags:
   - `inventory_app:<YYYYMMDD_HHMM>`
   - `inventory_app:latest`
5. exporta la imagen con `docker save`
6. comprime el resultado con `xz`

Artefacto generado:

```text
inventory_app_<YYYYMMDD_HHMM>.tar.xz
```

Importante:

- la extension real es `.tar.xz`, no `.tar.gz`
- `docker-compose.prod.yml` espera la imagen local `inventory_app:latest`
- el puerto publicado en produccion tambien es `8480:80`

### Produccion: despliegue en el servidor final

Si la imagen se construye en otra maquina y luego se despliega en el servidor final, el proceso completo es este.
...
Eso dejara un archivo similar a:

```text
inventory_app_20260527_1244.tar.xz
```

#### 2. Copiar el artefacto al servidor final

Ejemplo:

```bash
scp inventory_app_20260527_1244.tar.xz usuario@servidor:/ruta/deploy/
```

#### 3. Cargar la imagen Docker en el servidor final

En el servidor destino:

```bash
xz -dk inventory_app_20260527_1244.tar.xz
docker load -i inventory_app_20260527_1244.tar
```

Despues de `docker load`, la imagen `inventory_app:latest` quedara disponible localmente, que es justo la que usa `docker-compose.prod.yml`.

#### 4. Crear la red externa si no existe

```bash
docker network create h3_net || true
```

#### 5. Levantar el contenedor final en el puerto 8480
...
- imagen usada: `inventory_app:latest`
- puerto publicado: `8480`
- acceso HTTP: `http://<host>:8480`

#### 6. Verificacion y operacion basica
...
### Produccion: despliegue si el build se hace en el mismo servidor

Si el mismo servidor genera la imagen y tambien la ejecuta, no hace falta exportar/importar:
...
El flujo que hoy gobierna el despliegue productivo es `scripts/build_and_export.sh`. Conviene unificar ambos para evitar builds ambiguos.
